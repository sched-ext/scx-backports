...
static struct task_ctx *lookup_task_ctx(struct task_struct *p)
{
	...
	if (!taskc)
		__COMPAT_scx_bpf_error("task_ctx lookup failed for pid %d", p->pid);
...
int dom_xfer_task(pid_t pid, u32 new_dom_id, u64 now)
{
	...
	if (!p) {
		__COMPAT_scx_bpf_error("Failed to lookup task %d", pid);
		return 0;
	}
...
void BPF_STRUCT_OPS(rusty_running, struct task_struct *p)
{
	...
	if (taskc->dom_active_pids_gen != dap_gen) {
		...
		if (!pidp) {
			__COMPAT_scx_bpf_error("dom_active_pids[%u][%llu] indexing failed",
				      dom_id, idx);
			return;
		}
				     ...
				     bool init_dsq_vtime)
{
	...
	if (!task_set_domain(taskc, p, dom_id, init_dsq_vtime))
		__COMPAT_scx_bpf_error("Failed to set dom%d for %s[%d]",
			      dom_id, p->comm, p->pid);
}
